...
void display(void)
{
	...
	for(i=0; i<NF; i++)
	{
		...
		if(f->Type() == SPRING_FORCE)
		{
			...
			glColor3ub(sf->GetR(), sf->GetG(), sf->GetB());
...
void readFromFile(){
	...
	cin >> filename;
	*/
	char filename[100] = "colorSquare.txt";
	ifstream fin(filename);
	if(!fin){
		cout << "Unknown Filename" << std::endl;
	} else{
		while(!fin.eof()){
			char type[100];
			fin >> type >> ws;
			if(strcmp(type, "particle") == 0){
				...
				int fixed;
				fin >> x >> y >> dx >> dy >> rad >> fixed >> ws;
				Particle* p = new Particle(x, y, dx, dy, rad, fixed==1);
				PS.AddParticle(p);

			} else if (strcmp(type, "spring") == 0) {
				int p1, p2, r, g, b;
				double springiness, damping, restlength;
				fin >> p1 >> p2 >> springiness >> damping >> restlength >> r >> g >> b >> ws;
				SpringForce* s = new SpringForce( PS.GetParticle(p1), PS.GetParticle(p2), springiness, damping, restlength, r, g, b);
				PS.AddForce(s);

			} else if( strcmp(type, "DeltaT") == 0) {
				double DeltaT;
				fin >> DeltaT >> ws;
				PS.UpdateDT(DeltaT);
			}
